...
% new baseline 
        ...
        GEN_MATRIX = [0.00473950079288317,0,-0.00211019545093086,0.0153131811631311;0.00966888404424071,0,-0.0213730895087010,-0.0426029877934497;0.00615812993332741,0,-0.00383065315231201,-0.00481371580855891;0.00442445677080744,0,-0.0811603673420479,-0.0335072648900061];
    end 
    if button2 == 2 
...
% PEN plotting 
for q = 1:pencolnum %1-4
    row = MATRIX(q,:) ;
    p3 = plot(1:b, MATRIX(q, :), 'r', 'DisplayName','PEN data') ;
end
% SHAM plotting
for q = pencolnum+1:(pencolnum+shamcolnum) % 5-8
    row = MATRIX(q,:) ;
    p1 = plot(1:b, MATRIX(q, :), 'g', 'DisplayName','SHAM data') ;
end 
% GEN plotting 
for q = pencolnum+shamcolnum+1:(pencolnum+shamcolnum+gencolnum) %9-12
    row = MATRIX(q,:) ;
